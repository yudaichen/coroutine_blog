...
BlogMapper::BlogMapper(mysql::connection_pool &pool)
{
    pool_ = &pool;
}

asio::awaitable<void> BlogMapper::get_articles_full_text_awaitable(std::string &full_text, int page, int page_size,
                                                                   std::vector<fast::entity::BlogArticle> &articles)
{
    try
    {
        ...
        query << "SELECT article_id, title, content FROM "
              << "blog_article WHERE MATCH (title, markdown) AGAINST ('" << full_text << "' IN NATURAL LANGUAGE MODE) "
        ...
        for (auto &&row : result.rows())
        {
            ...
            article.content_preview = fast::util::remove_html_tags(row.at(2).as_string());
            if (article.content_preview.length() > 30)
            {
                article.content_preview = fast::util::split_first_n_unicode_chars(article.content_preview, 40) + "...";
            }
    ...
    {
        std::cerr << "General Exception in get_articles_full_text_awaitable: " << e.what() << std::endl;
    }
    co_return;
}

asio::awaitable<void> BlogMapper::get_articles_by_column_id_awaitable(std::uint64_t column_id, int page, int page_size,
                                                                      std::vector<fast::entity::BlogArticle> &articles)
{
    try
    {
        ...
        if (column_id == 0)
        {
            co_await conn->async_execute(mysql::with_params("SELECT article_id, title, content FROM "
                                                            "blog_article LIMIT {}, {}",
                                                            offset, page_size),
        ...
        for (auto &&row : result.rows())
        {
            ...
            if (article.content_preview.length() > 30)
            {
                article.content_preview = util::split_first_n_unicode_chars(article.content_preview, 40) + "...";
            }
            articles.emplace_back(article);
        }
    }
    ...
    {
        std::cerr << "General Exception in get_articles_by_column_id_awaitable: " << e.what() << std::endl;
    }
    co_return;
}

asio::awaitable<void> BlogMapper::get_all_blog_columns_awaitable(std::vector<fast::entity::BlogColumn> &columns)
{
    try
    {
        mysql::pooled_connection conn = co_await pool_->async_get_connection(asio::use_awaitable);
    ...
    {
        std::cerr << "BlogMapper Error (system_error): " << ec.code().message() << " (" << ec.code().value() << ")"
                  << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "General Exception in get_all_blog_columns_awaitable: " << e.what() << std::endl;
    }
}

asio::awaitable<void> BlogMapper::get_article_by_article_id_awaitable(std::uint64_t article_id,
                                                                      fast::entity::BlogArticle &article)
{
    try
    {
        ...
        mysql::results result;
        mysql::diagnostics diag;
        co_await conn->async_execute(mysql::with_params("SELECT title, markdown, create_time, update_time "
                                                        "FROM blog_article "
                                                        "WHERE article_id = {}",
                                                        article_id),
                                     result, diag, asio::use_awaitable);
...
asio::awaitable<void> BlogMapper::watch_number_added_awaitable(uint64_t article_id)
{
    ...
    co_await conn->async_execute(mysql::with_params("UPDATE blog_article "
