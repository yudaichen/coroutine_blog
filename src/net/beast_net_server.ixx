...
concept ValidStream =
    std::is_same_v<Stream, beast::tcp_stream> || std::is_same_v<Stream, asio::ssl::stream<beast::tcp_stream>>;
...
class task_group
{
  ...
  public:
    ...
    template <typename CompletionToken> auto adapt(CompletionToken &&completion_token);
...
class Server
{
  public:
    ...
    Server(const asio::address &address, unsigned short port, const std::string &doc_root, int threads,
  ...
  private:
    ...
    fast::net::SessionManager session_;

    asio::awaitable<void> detect_session(beast::tcp_stream stream, asio::ssl::context &ctx, beast::string_view doc_root,
                                         Protocol protocol);

    template <ValidStream Stream>
    static asio::awaitable<void> run_session(Stream &stream, beast::flat_buffer &buffer, beast::string_view doc_root,
                                             Server &server);

    template <ValidStream Stream>
    static asio::awaitable<void> run_websocket_session(Stream &stream, beast::flat_buffer &buffer,
                                                       beast::http::request<beast::http::string_body> req,
                                                       beast::string_view doc_root, Server &server);

    asio::awaitable<void> listen(task_group &task_group, asio::ssl::context &ctx, asio::net::tcp::endpoint endpoint,
