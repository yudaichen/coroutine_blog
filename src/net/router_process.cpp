...
asio::awaitable<void>
Router::handle_request(beast::http::request<beast::http::string_body> &req,
  ...
  for (const auto &seg : path_segments) {
    ...
    for (const auto &child : current->children) {
      if (child.first == seg) {
        ...
        found = true;
        break;
      } else if (!child.first.empty() && child.first[0] == '{' &&
                 child.first.back() == '}') {
        current = child.second.get();
...
Router::parse_query_params(beast::string_view query) {
...
template <typename H>
void Router::add_route_internal(const std::string &method,
                                const std::string &path, H handler) {
  std::vector<std::string> path_segments;
...
void Router::add_route_rest_full(const std::string &method,
